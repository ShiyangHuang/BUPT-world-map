      ...
      return {
        controller: function ($scope, $http) {
          ...
          $scope.submit = function () {
            if ($scope.userinfo.username == null || $scope.userinfo.email == null) return;
            if ("geolocation" in navigator) {
              navigator.geolocation.getCurrentPosition(function (position) {
                var c = position.coords;
                $scope.mylocation = c;
                $scope.userinfo.lat = c.latitude;
                $scope.userinfo.lng = c.longitude;
            ...
            {
              alert("Hello");
              $scope.userinfo.lat = 39.65411075698885;
              $scope.userinfo.lng = -93.26246782812503;
              $http.post(postURL,$scope.userinfo).success(function(data) {
                  console.log("success");
                }).error(function() {
                  console.log("error");
